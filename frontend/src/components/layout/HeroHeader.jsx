...
const HeroHeader = ({ title, subtitle, action, breadcrumb }) => {
  ...
  const expiringSoon = notifications.filter((subscription) => daysUntilExpiry(subscription.expiryDate) >= 0);
  const recentlyExpired = notifications.filter((subscription) => daysUntilExpiry(subscription.expiryDate) < 0);
                  ...
                  {notifications.length === 0 ? (
                    ...
                    <ul className={styles.notifList}>
                      ...
                      {expiringSoon.map((subscription) => (
                        <li key={subscription.id} className={styles.notifItem}>
                          <div className={styles.notifContent}>
                            <p className={styles.notifTitle}>{subscription.serviceName}</p>
                            <p className={styles.notifSub} style={{ color: 'var(--warning-600)' }}>
                              {expiryLabel(subscription.expiryDate)}
                            </p>
                          </div>
                        </li>
                      ))}
                      {recentlyExpired.length > 0 && <li className={styles.notifSection}>RECENTLY EXPIRED</li>}
                      {recentlyExpired.map((subscription) => (
                        <li key={subscription.id} className={styles.notifItem}>
                          <div className={styles.notifContent}>
                            <p className={styles.notifTitle}>{subscription.serviceName}</p>
